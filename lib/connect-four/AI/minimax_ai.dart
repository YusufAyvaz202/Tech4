  ...
  List<int> getTopTwoMoves(List<List<int>> board) {
    ...
    for (int col in validLocations) {
      ...
      if (_checkWinningMove(bCopy, playerPiece)) {
        ...
        bool givesAiWin = false;
        List<int> nextValidLocations = _getValidLocations(bCopy);
        
        for (int nextCol in nextValidLocations) {
          List<List<int>> nextCopy = _copyBoard(bCopy);
          _dropPiece(nextCopy, nextCol, aiPiece);
          
          if (_checkWinningMove(nextCopy, aiPiece)) {
            givesAiWin = true;
            break;
          }
        }
